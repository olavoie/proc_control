...
namespace proc_control
{
    ...
    TargetMode::TargetMode(std::shared_ptr<RobotState> &robotState, std::unique_ptr<ControllerIF> &controlAUV):
    ...
    void TargetMode::SetTarget(bool isGlobal, Eigen::VectorXd &targetPose)
    {
        robotState_->SetDesiredTwist(targetPose);
        robotState_->TwistPublisher(targetPose, robotState_->GetVelocityTargetPublisher());
    }

    void TargetMode::SetDecoupledTarget(bool isGlobal, const std::vector<bool> &keepTarget, Eigen::VectorXd &targetPose)
    {
    }

    void TargetMode::GetLocalError(Eigen::VectorXd &targetPose, Eigen::VectorXd &localError)
    {
